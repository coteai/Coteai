  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="premium-title text-3xl md:text-4xl uppercase tracking-tighter mb-2">Gestão de Equipe</h1>
          <p className="text-slate-400">Cadastre consultores e controle o acesso deles à plataforma.</p>
        </div>
      ...
      {/* FILTER TRAY */}
      <div className="glass-card p-4 rounded-xl flex items-center space-x-4 border border-white/5">
        <div className="relative flex-1 max-w-sm">
          ...
          className="w-full bg-black/40 border border-white/10 rounded-lg pl-10 pr-4 py-2.5 text-white text-sm focus:outline-none focus:border-white/30 transition-all font-medium"
          />
        </div>
      ...
      <div className="glass-panel overflow-hidden rounded-2xl relative">
        <table className="w-full text-left border-collapse min-w-[600px]">
          <thead>
            <tr>
              <th className="p-4 border-b border-white/5 bg-white/5 text-xs font-black text-zinc-500 uppercase tracking-widest">Consultor</th>
              <th className="p-4 border-b border-white/5 bg-white/5 text-xs font-black text-zinc-500 uppercase tracking-widest hidden md:table-cell">Contato</th>
              <th className="p-4 border-b border-white/5 bg-white/5 text-xs font-black text-zinc-500 uppercase tracking-widest">Status</th>
              <th className="p-4 border-b border-white/5 bg-white/5 text-xs font-black text-zinc-500 uppercase tracking-widest text-right">Ações</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={4} className="p-16 text-center">
                  ...
                  <p className="text-white font-bold text-lg">Nenhum consultor encontrado</p>
                  <p className="text-zinc-500">Cadastre sua equipe para que eles gerem cotações.</p>
                </td>
              </tr>
            ) : (
                ...
                <tr key={consultant.id} className="hover:bg-white/[0.02] transition-colors border-b border-white/5 group">
                  ...
                  <td className="p-4">
                    <div className="flex items-center space-x-4">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border uppercase font-black text-sm
                      ...
                      <div className="flex flex-col">
                        <span className="font-bold text-white mb-0.5">{consultant.nome}</span>
                        <span className="text-xs text-zinc-500">Adicionado em {new Date(consultant.created_at).toLocaleDateString('pt-BR')}</span>
                      </div>
                    </div>
                  </td>
                  
                  {/* WhatsApp/Email (Hidden on mobile) */}
                  <td className="p-4 hidden md:table-cell">
                    <div className="flex flex-col">
                      <span className="text-sm font-medium text-zinc-300">{consultant.whatsapp || 'sem whatsapp'}</span>
                      <span className="text-xs text-zinc-500 font-mono">{consultant.email || 'sem e-mail'}</span>
                    </div>
                  </td>
                  ...
                  <td className="p-4">
                    {consultant.ativo ? (
                      <span className="inline-flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 text-xs font-bold text-emerald-400 uppercase tracking-widest">
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></div>
                        <span>Ativo</span>
                      </span>
                    ) : (
                      <span className="inline-flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg border border-zinc-700/50 bg-white/5 text-xs font-bold text-zinc-600 uppercase tracking-widest">
                  ...
                  <td className="p-4 text-right">
                    <div className="flex justify-end items-center space-x-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        ...
                        title={consultant.ativo ? 'Bloquear Acesso' : 'Liberar Acesso'}
                      >
                        <CheckCircle size={16} />
                      </button>
                      <button 
                        onClick={() => setEditingConsultant(consultant)}
                        className="p-2 text-indigo-400 hover:bg-indigo-500/10 border border-transparent hover:border-indigo-500/20 rounded-lg transition-colors"
                      >
                        <Edit3 size={16} />
                      </button>
                      <button 
                        onClick={() => handleDelete(consultant.id)}
                        className="p-2 text-red-500/50 hover:bg-red-500/10 hover:text-red-400 rounded-lg transition-colors"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
