...
const QuoteGenerator = () => {
  ...
  const cachedPdfBlobRef = useRef<Blob | null>(null);
  const isPreGeneratingRef = useRef<boolean>(false);

  // Invalida o PDF em cache se os planos ou coberturas forem alterados
  useEffect(() => {
    cachedPdfBlobRef.current = null;
  }, [availablePlans, editedCoberturas, editedMensalidade, editedAdesao, formData, intelligence.data]);

  // Constroi o PDF percorrendo as paginas com timeout de seguranca
  const buildPdfBlob = async (): Promise<Blob | null> => {
    ...
    try {
      ...
      for (let i = 0; i < childrenNodes.length; i++) {
        ...
        const isDark = pageElement?.getAttribute?.('data-theme') === 'dark' || 
                       pageElement?.classList?.contains?.('dark-pdf-page');
        const canvasBg = isDark ? '#080F1E' : '#ffffff';

        let imgData: string;
        try {
          // Captura visual rapida com timeout de 8s
          const renderPromise = htmlToImage.toPng(pageElement, {
            backgroundColor: canvasBg,
            pixelRatio: 1.5,
            cacheBust: true,
          });
          const timeoutPromise = new Promise<string>((_, reject) =>
            setTimeout(() => reject(new Error('Timeout htmlToImage')), 8000)
          );
          imgData = await Promise.race([renderPromise, timeoutPromise]);
        } catch (htmlErr) {
          console.warn('[PDF] htmlToImage falhou ou demorou, usando fallback html2canvas:', htmlErr);
      ...
      console.error('[PDF] Erro ao construir PDF:', err);
      return null;
    }
  };

  // Pre-geracao em background assim que o consultor chega no Step 4 (Resumo)
  useEffect(() => {
    if (step === 4 && pdfRef.current && availablePlans.length > 0) {
      const timer = setTimeout(async () => {
        if (cachedPdfBlobRef.current || isPreGeneratingRef.current) return;
        ...
        try {
          ...
          console.warn('[PDF] Pre-geracao silenciosa em background falhou:', e);
        } finally {
  ...
  }, [step, availablePlans, intelligence.status]);

  const generatePdfBlob = async (): Promise<Blob | null> => {
    if (cachedPdfBlobRef.current) {
      return cachedPdfBlobRef.current;
    }
  ...
  const handleNativeShare = async () => {
    let blob = cachedPdfBlobRef.current;
    if (!blob) {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        ...
        console.warn('Share nativo falhou, usando fallback:', err);
      }
    }
  ...
  const handleWhatsAppShare = async () => {
    ...
    // Em dispositivos moveis que suportam envio direto de arquivo no share: abre a tela de envio do PDF
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({
          files: [file],
        ...
        if (err?.name === 'AbortError') return;
        console.warn('Share nativo WhatsApp falhou, tentando fallback WhatsApp Web:', err);
      }
    }

    // No desktop/PC ou se o share de arquivos nao for suportado pelo navegador:
    // Baixa o PDF automaticamente e abre o WhatsApp com os dados da cotacao
    handleDownloadPDF(blob);
  ...
  const handleDownloadPDF = async (preGeneratedBlob = null) => {
    const blob = preGeneratedBlob || cachedPdfBlobRef.current || await generatePdfBlob();
    if (!blob) return;
