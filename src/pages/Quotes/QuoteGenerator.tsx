...
const QuoteGenerator = () => {
  ...
  const currentStepNum = typeof step === 'number' ? step : 2;

  const renderStepIcon = (num: number, icon: any, label: string) => {
    const isCurrent = currentStepNum === num;
    const isDone = currentStepNum > num;
    return (
      <div className="flex flex-col items-center">
        <div
          className={`relative w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 bg-[var(--color-surface)] ${
            isCurrent
              ? 'border-white text-white scale-105 shadow-[0_0_16px_rgba(255,255,255,0.25)]'
              : isDone
              ? `${theme.colors.border} ${theme.colors.primary}`
              : 'border-white/10 text-zinc-600'
          }`}
          style={isDone ? { boxShadow: `0 0 12px ${theme.colors.shadow}` } : {}}
        >
          <div
            className={`absolute inset-0 rounded-full ${isCurrent ? 'bg-white/5' : ''}`}
            style={isDone ? { backgroundColor: `${theme.colors.glowHex}1A` } : {}}
          />
          <div className="relative z-10">
            {isDone ? (
              <CheckCircle2 size={16} className={`${theme.colors.primary} sm:w-[18px] sm:h-[18px]`} />
            ) : (
              icon
            )}
          </div>
        ...
        <span
          className={`mt-1 sm:mt-1.5 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider ${
            isCurrent ? 'text-white' : isDone ? theme.colors.primary : 'text-zinc-500'
          }`}
        >
          {label}
        </span>
  ...
  return (
    <div className="flex flex-col h-full max-w-6xl mx-auto w-full gap-2 sm:gap-3 md:gap-3.5">
      ...
      {/* Stepper Compacto & Proporcional */}
      <div className="bg-[var(--color-surface)]/80 backdrop-blur-xl border border-white/5 rounded-xl sm:rounded-2xl px-4 sm:px-8 py-2 sm:py-2.5 relative shrink-0">
        {/* Progress track */}
        <div className="absolute top-[22px] sm:top-[26px] left-[10%] right-[10%] h-px bg-white/10 z-0" />
        <div
          className="absolute top-[22px] sm:top-[26px] left-[10%] h-px z-0 transition-all duration-500"
          style={{
            backgroundColor: theme.colors.glowHex,
            width: `${Math.max(0, [1, 2, 3, 4].indexOf(currentStepNum)) * (80 / 3)}%`,
          }}
        />
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            ...
            >
              ...
              <div className="flex justify-center mb-4 sm:mb-5 bg-[#141f38]/80 p-1 sm:p-1.5 rounded-xl sm:rounded-2xl border border-white/5 mx-auto w-full max-w-md">
                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, tipo_veiculo: 'carro' })}
                  className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2 sm:py-2.5 px-2 rounded-lg sm:rounded-xl transition-all ${
                    formData.tipo_veiculo === 'carro'
                      ? `text-white border ${theme.colors.border}`
                      : 'text-zinc-400 hover:text-zinc-200'
                  }`}
                  style={
                    formData.tipo_veiculo === 'carro'
                      ? { backgroundColor: `${theme.colors.glowHex}33`, boxShadow: `0 0 15px ${theme.colors.shadow}` }
                      : {}
                  }
                >
                  <Car size={16} />
                ...
                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, tipo_veiculo: 'moto' })}
                  className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2 sm:py-2.5 px-2 rounded-lg sm:rounded-xl transition-all ${
                ...
                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, tipo_veiculo: 'caminhao' })}
                  className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2 sm:py-2.5 px-2 rounded-lg sm:rounded-xl transition-all ${
              ...
              {!manualMode && (
                <div className="w-full max-w-md mx-auto space-y-3">
                  {/* Grid Responsivo: Placa + UF lado a lado */}
                  <div className="grid grid-cols-1 sm:grid-cols-12 gap-2.5 sm:gap-3 text-left">
                    <div className="sm:col-span-8 flex flex-col">
                      <label className="block text-[11px] font-black text-zinc-400 uppercase tracking-widest mb-1">
                        Placa
                      </label>
                      <div className="relative">
                        <div className="absolute top-0 left-0 h-full w-3.5 bg-blue-700 rounded-l-xl flex flex-col items-center justify-between py-2 overflow-hidden border border-blue-900 border-r-0 z-10">
                        ...
                        <input
                          ...
                          className="w-full h-12 sm:h-14 bg-[#141f38] border-2 border-white/10 rounded-xl pl-6 pr-2 text-2xl sm:text-3xl text-center text-white focus:outline-none focus:border-blue-400 font-bold tracking-[0.2em] shadow-[0_0_20px_rgba(59,130,246,0.05)] font-mono uppercase"
                          placeholder="AAA0A00"
                          maxLength={7}
                        />
                      </div>
                    ...
                    <div className="sm:col-span-4 flex flex-col">
                      ...
                      <select
                        ...
                        className="w-full h-12 sm:h-14 bg-[#141f38] border-2 border-white/10 rounded-xl px-3 text-sm sm:text-base text-white font-bold focus:outline-none focus:border-blue-400 transition-colors appearance-none cursor-pointer"
                      >
                  ...
                  <button
                    ...
                    className={`w-full flex justify-center items-center h-12 sm:h-13 rounded-xl text-white font-black uppercase tracking-widest transition-all text-xs sm:text-sm ${
                      formData.placa.length >= 7 && formData.uf
                        ? theme.colors.bg
                        : 'bg-white/5 text-zinc-600 cursor-not-allowed'
                    }`}
                    style={
                      formData.placa.length >= 7 && formData.uf
                        ? { boxShadow: `0 0 20px ${theme.colors.shadow}` }
                        : {}
                    }
                  >
                    {loading ? (
                      ...
                      <span>
                        Buscar Variantes FIPE <ArrowRight className="inline-block ml-2 w-4 h-4" />
                      </span>
                    )}
                  </button>
                </div>
              ...
              {/* Alternador Modo Manual */}
              <div className="flex justify-center mt-3">
                <button
                  type="button"
                  onClick={() => { setManualMode(!manualMode); setError(''); }}
                  className={`flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider px-3.5 py-1.5 rounded-full border transition-all ${
                    manualMode
              ...
              {manualMode && (
                <div className="mt-3 mx-auto w-full max-w-md space-y-2.5 bg-amber-500/5 border border-amber-500/20 rounded-xl p-3.5 sm:p-4 text-left">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-black/60 border border-white/10 rounded-xl px-3 py-2 text-xs sm:text-sm text-white font-bold focus:outline-none focus:border-amber-400 transition-colors"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1">
                      ...
                      <div className="relative">
                        <DollarSign size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-zinc-500" />
                        <input
                          type="number"
                          min={1000}
                          max={1000000}
                          ...
                          className="w-full bg-black/60 border border-white/10 rounded-xl pl-8 pr-3 py-2 text-xs sm:text-sm text-white font-mono font-bold focus:outline-none focus:border-amber-400 transition-colors"
                        />
                      </div>
                    </div>
                  </div>
                  <button
                    ...
                    className="w-full flex justify-center items-center gap-1.5 py-2.5 rounded-xl text-black font-black uppercase tracking-widest text-xs transition-all bg-amber-400 hover:bg-amber-300 disabled:opacity-40 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(245,158,11,0.25)]"
                  >
                    {loading ? (
              ...
              {error && (
                <div className="mt-4 flex items-center justify-center space-x-2 text-red-400 text-xs sm:text-sm font-medium">
...
{/* STEP 2: VERSÕES FIPE */}
          {step === 2 && (
            <motion.div
              ...
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: -20 }}
              className="p-3 sm:p-5 md:p-6 flex-1 flex flex-col max-w-5xl mx-auto w-full min-h-0"
            >
              <div className="flex justify-between items-center mb-3 pb-2 border-b border-white/5 shrink-0">
                <div>
                  ...
                  <p className="text-zinc-400 text-xs sm:text-sm">
                    ...
                    <span className="ml-1 text-zinc-300 text-[11px] font-bold uppercase tracking-wider bg-white/5 px-2 py-0.5 rounded-full">
                      {fipeVariants.length} encontradas
                    </span>
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="text-zinc-300 text-xs sm:text-sm hover:underline font-bold bg-white/5 px-3 py-1.5 rounded-lg border border-white/10 transition-colors shrink-0"
                >
                  Voltar
                </button>
              </div>
              ...
              {error && (
                <div className="flex items-center space-x-2 bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl px-3.5 py-2 mb-3 text-xs sm:text-sm font-medium shrink-0">
                  <AlertCircle size={16} className="shrink-0" />
                  <span>{error}</span>
                </div>
              )}

              {/* Grid Responsivo de Variantes: 2 colunas em telas maiores */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 sm:gap-3 flex-1 min-h-0 overflow-y-auto pr-1 sm:pr-2 custom-scrollbar pb-2">
                {fipeVariants.map((v) => (
                  <button
                    key={v.id}
                    onClick={() => selectVariantAndShowGroups(v)}
                    disabled={loading}
                    className="w-full bg-white/[0.02] hover:bg-white/5 border border-white/5 hover:border-white/20 rounded-xl p-3 sm:p-3.5 flex items-center justify-between gap-3 text-left group transition-all"
                  >
                    <div className="flex items-center min-w-0">
                      <div
                        className="w-9 h-9 rounded-lg border flex items-center justify-center mr-3 transition-colors shrink-0"
                        style={{
                          backgroundColor: `${theme.colors.glowHex}1A`,
                          borderColor: `${theme.colors.glowHex}50`,
                      ...
                      <div className="min-w-0">
                        <h3 className="text-xs sm:text-sm font-bold text-zinc-300 group-hover:text-white transition-colors uppercase leading-snug line-clamp-2">
                          {v.modelo}
                        </h3>
          ...
          {step === '2b' && (
            <motion.div
              ...
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: -20 }}
              className="p-3 sm:p-5 md:p-6 flex-1 flex flex-col max-w-4xl mx-auto w-full min-h-0"
            >
              <div className="flex justify-between items-center mb-3 pb-2 border-b border-white/5 shrink-0">
                <div>
                  <h2 className="premium-title text-lg sm:text-2xl uppercase tracking-tighter mb-0.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1 min-h-0 overflow-y-auto pr-1">
                {vehicleGroups
                  .filter((g) => g.base_type === formData.tipo_veiculo)
                  .map((group) => (
                    <button
                      key={group.id}
                      onClick={() => fetchPricingForGroup(selectedVariant, group.id)}
                      disabled={loading}
                      className="w-full bg-white/[0.02] hover:bg-white/5 border border-white/5 hover:border-white/20 rounded-xl p-4 flex items-center justify-between text-left group transition-all disabled:opacity-50"
                    >
                      <div className="flex items-center min-w-0">
                        <div
                          className="w-11 h-11 rounded-xl border flex items-center justify-center shrink-0 mr-3.5 transition-colors"
                          style={{
                            backgroundColor: `${theme.colors.glowHex}1A`,
                            borderColor: `${theme.colors.glowHex}50`,
                            boxShadow: `0 0 12px ${theme.colors.shadow}`,
                        ...
                        >
                          {group.base_type === 'carro' ? (
                            <Car size={22} className="text-white" />
                          ) : group.base_type === 'moto' ? (
                        ...
                        <div className="min-w-0">
                          <h3 className="text-sm sm:text-base font-black text-zinc-300 group-hover:text-white transition-colors">
                            {group.nome}
                          </h3>
                          <p className="text-[10px] text-zinc-400 font-bold bg-white/5 inline-block px-2 py-0.5 rounded mt-1 border border-white/5">
                            Selecionar categoria
                          </p>
                        </div>
                      </div>
                      {loading && selectedGroupId === group.id && (
                        <Loader2 className="animate-spin text-white ml-2" size={18} />
                      )}
                    </button>
                  ))}
              </div>
            ...
            >
              ...
              <div className="pt-2.5 sm:pt-3 border-t border-white/10 shrink-0 bg-[var(--color-surface)]/90 backdrop-blur-md">
                <button
                  ...
                  disabled={saving}
                  className={`w-full py-3 sm:py-3.5 rounded-xl font-black uppercase tracking-widest text-xs sm:text-sm transition-all text-white disabled:opacity-60 flex justify-center items-center ${theme.colors.bg}`}
                  style={{ boxShadow: `0 0 20px ${theme.colors.shadow}` }}
                >
                  {saving ? <Loader2 className="animate-spin" size={18} /> : 'GERAR PROPOSTA'}
                </button>
              </div>
            </motion.div>
          )}
            ...
            >
              ...
              <div className="flex items-center justify-between pb-2.5 mb-3 border-b border-white/10 shrink-0">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h2 className="premium-title text-base sm:text-xl uppercase tracking-tight">
                      Proposta Gerada com Sucesso!
                    </h2>
                    <p className="text-[11px] sm:text-xs text-zinc-400">
                      Pronta para envio ao cliente ou download em PDF.
                    </p>
                  </div>
                </div>
                <button
                ...
                <div className="lg:col-span-5 flex flex-col gap-3 shrink-0">
                        ...
                        return (
                          <div
                            ...
                            className={`p-2.5 sm:p-3 rounded-xl border ${
                              isVip
                                ? 'border-blue-500/30 bg-blue-500/10 shadow-[0_0_15px_rgba(37,99,235,0.1)]'
                                : 'border-white/10 bg-white/5'
                            }`}
                          >
                            <p
                              className={`font-black uppercase text-xs mb-0.5 flex items-center ${
                                isVip ? 'text-blue-400' : 'text-white'
                              }`}
                            >
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:gap-2.5">
                    <button
                      ...
                      className={`flex flex-col items-center justify-center bg-[#25D366]/15 hover:bg-[#25D366] text-[#25D366] hover:text-white border border-[#25D366]/40 py-2.5 sm:py-3 rounded-xl font-bold transition-all group ${
                        isGeneratingPDF
                          ? 'opacity-50 cursor-not-allowed'
                          : 'shadow-[0_0_15px_rgba(37,211,102,0.15)]'
                      }`}
                    >
                      {isGeneratingPDF ? (
                      ...
                      <span className="text-[10px] uppercase tracking-wider font-black">WhatsApp</span>
                    </button>
                    ...
                    <button
                      ...
                      onClick={handleNativeShare}
                      disabled={isGeneratingPDF}
                      className={`flex flex-col items-center justify-center bg-white/5 hover:bg-blue-600 hover:border-blue-600 hover:text-white text-zinc-300 border border-white/10 py-2.5 sm:py-3 rounded-xl font-bold transition-all group ${
                    ...
                    >
                      {isGeneratingPDF ? (
                        ...
                        <Share2 className="w-5 h-5 mb-1 text-zinc-400 group-hover:text-white" />
                      )}
                      <span className="text-[10px] uppercase tracking-wider font-black">Compartilhar</span>
                    </button>
                    ...
                    <button
                      ...
                      disabled={isGeneratingPDF}
                      className={`flex flex-col items-center justify-center bg-white/5 hover:bg-white hover:text-black hover:border-white text-zinc-300 border border-white/10 py-2.5 sm:py-3 rounded-xl font-bold transition-all group ${
                        isGeneratingPDF ? 'opacity-50 cursor-not-allowed' : ''
                      }`}
                    >
                      {isGeneratingPDF ? (
                        ...
                        <Download className="w-5 h-5 mb-1 text-zinc-400 group-hover:text-black" />
                      )}
                      <span className="text-[10px] uppercase tracking-wider font-black">PDF Direto</span>
                    </button>
                  </div>
                </div>
