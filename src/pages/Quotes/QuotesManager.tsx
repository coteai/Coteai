  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="premium-title text-3xl md:text-4xl uppercase tracking-tighter mb-2">Central de Negócios</h1>
          <p className="text-slate-400">Gerencie as cotações geradas e aprove contratos para pontuar consultores.</p>
        </div>
        <div className="flex items-center space-x-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Buscar placa, cliente, consultor..."
              className="pl-10 pr-4 py-2 bg-black/60 border border-white/10 rounded-xl text-white placeholder:text-zinc-600 focus:border-white/30 focus:outline-none w-72 font-medium transition-colors"
            />
          </div>
          <button
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((quote, idx) => (
                  ...
                  className={`border-b border-white/5 group transition-colors ${
                    quote.status === 'pending' ? 'hover:bg-emerald-950/20' :
                    quote.status === 'converted' ? 'hover:bg-white/[0.02]' :
                    'hover:bg-white/[0.02]'
                  }`}
                  >
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      ...
                      {quote.valor_fipe && <div className="text-xs text-zinc-600 mt-0.5">FIPE: {formatCurrency(quote.valor_fipe)}</div>}
                    </td>
                    <td className="p-4">
                      {quote.consultants ? (
                    ...
                    <td className="p-4">
                      <div className="font-black text-white mb-1">
                        {quote.plano_selecionado === 'Múltiplas Opções' && <span className="text-xs text-zinc-500 font-normal mr-1">a partir de</span>}
